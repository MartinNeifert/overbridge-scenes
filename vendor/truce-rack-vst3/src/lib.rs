...
use vst3::Steinberg::Vst::{
    ...
    IHostApplication, IParameterChanges, NoteOffEvent, NoteOnEvent, ParamID,
    ParamValue, ParameterInfo as Vst3ParameterInfo,
...
use vst3::Steinberg::{
    ...
    kPlatformTypeNSView, kPlatformTypeX11EmbedWindowID, kResultOk, kResultTrue, tresult,
};
...
/// Map a channel count to a VST3 speaker-arrangement bitmask.
/// Overbridge devices expose 4-in/4-out HAL endpoints; quad (L|R|Ls|Rs)
/// is the usual arrangement for those plugins.
fn speaker_arrangement_for_channels(channels: u32) -> u64 {
    match channels {
        0 | 1 => 0x01,
        2 => STEREO_ARRANGEMENT,
        4 => 0x33, // kSpeakerL | kSpeakerR | kSpeakerLs | kSpeakerRs
        n if n <= 63 => (1u64 << n) - 1,
        _ => STEREO_ARRANGEMENT,
    }
}
...
pub struct Vst3Plugin {
    ...
    /// Host-side services kept alive for the plugin lifetime.
...
impl Vst3Plugin {
    fn load_from(info: &PluginInfo) -> Result<Self> {
        ...
        let host_app = ComWrapper::new(HostApplication::default());
        let host_context = host_app
            .to_com_ptr::<IHostApplication>()
            .map(|p| p.as_ptr().cast())
            .unwrap_or(ptr::null_mut());

        // Initialize the component with a host application context.
        if unsafe { component.initialize(host_context) } != kResultOk {
            return Err(Error::LoadFailed {
                path: info.path.clone(),
        ...
        let (controller, separate_controller) = if cls_id_status == kResultTrue {
            ...
            if unsafe { ctrl_ptr.initialize(host_context) } != kResultOk {
                return Err(Error::LoadFailed {
                    path: info.path.clone(),
        ...
        if let Some(handler) = component_handler.to_com_ptr::<IComponentHandler>() {
            unsafe {
        ...
        // Skip createView probe — opening the view twice (probe + editor) can
        // crash JUCE/Overbridge plugins. Assume editor exists; open later on
        // the main thread after audio starts.
...
impl Vst3Plugin {
    /// Read `IEditController::getState` (parameter/UI side of the plugin).
    pub fn save_controller_state(&self) -> Result<Vec<u8>> {
        read_stream_state(|stream_ptr| unsafe { self.controller.getState(stream_ptr.as_ptr()) })
    }

    /// Write `IEditController::setState`.
    pub fn load_controller_state(&mut self, bytes: &[u8]) -> Result<()> {
        write_stream_state(bytes, |stream_ptr| unsafe {
            self.controller.setState(stream_ptr.as_ptr())
        })
    }

    /// Push processor/component state into the edit controller via
    /// `IEditController::setComponentState` — the documented host-on-load
    /// sequence (`component.getState` → `controller.setComponentState`).
    ///
    /// Overbridge applies hardware preset loads asynchronously into the
    /// processor without emitting `performEdit`. Empirically this call is what
    /// makes the controller refresh its parameter values (and emit
    /// `restartComponent(kParamValuesChanged)`) so `getParamNormalized` returns
    /// the new preset. Returns the raw `tresult`: Overbridge reports a non-OK
    /// status here even though the refresh takes effect, so callers should treat
    /// a non-OK return as informational, not fatal.
    pub fn push_component_state_to_controller(&self, component_bytes: &[u8]) -> tresult {
        let stream = match ComWrapper::new(MemoryStream {
            data: std::cell::RefCell::new(component_bytes.to_vec()),
...
impl PluginCore for Vst3Plugin {
    ...
    fn parameter_value_string(&self, index: usize, value: f64) -> Result<String> {
        ...
        let status = unsafe {
            self.controller
                .getParamStringByValue(info.id, value, &mut buf)
        };
        if status == kResultOk {
    ...
    fn set_parameter(&mut self, index: usize, value: f64) -> Result<()> {
        ...
        if let Some(host_editing) = self.controller.as_com_ref().cast::<IEditControllerHostEditing>()
        {
            unsafe {
                host_editing.beginEditFromHost(info.id);
            }
        }

        // Deliver via IParameterChanges in process(); also update controller state.
        ...
        {
            unsafe {
                host_editing.endEditFromHost(info.id);
            }
        }
    ...
    fn save_state(&self) -> Result<Vec<u8>> {
        read_stream_state(|stream_ptr| unsafe { self.component.getState(stream_ptr.as_ptr()) })
    }

    fn load_state(&mut self, bytes: &[u8]) -> Result<()> {
        write_stream_state(bytes, |stream_ptr| unsafe {
            self.component.setState(stream_ptr.as_ptr())
        })
    }
...
impl Plugin<f32> for Vst3Plugin {
    fn process(
        ...
        let pending_changes = std::mem::take(&mut self.pending_param_changes);
        // Coalesce: keep only the latest value per ParamID.
        let mut latest: std::collections::HashMap<ParamID, ParamValue> =
            std::collections::HashMap::new();
        for (id, value) in pending_changes {
            latest.insert(id, value);
        }
        let coalesced: Vec<(ParamID, ParamValue)> = latest.into_iter().collect();
        let param_changes_wrapper = if coalesced.is_empty() {
            None
        } else {
            Some(ComWrapper::new(ParameterChangesList::from_changes(
                coalesced,
            )))
        };
        let input_param_changes_ptr = param_changes_wrapper
            .as_ref()
...
fn read_stream_state<F>(read: F) -> Result<Vec<u8>>
where
    F: FnOnce(ComPtr<IBStream>) -> tresult,
{
    ...
    let status = read(stream_ptr);
    if status != kResultOk {
        return Err(Error::Other(format!("IBStream getState returned {status}")));
    }
    Ok(stream.data.borrow().clone())
}

fn write_stream_state<F>(bytes: &[u8], write: F) -> Result<()>
where
    F: FnOnce(ComPtr<IBStream>) -> tresult,
{
    ...
    let stream_ptr = stream
        .to_com_ptr::<IBStream>()
        .ok_or_else(|| Error::Other("MemoryStream missing IBStream IID".into()))?;
    let status = write(stream_ptr);
    if status != kResultOk {
        return Err(Error::Other(format!("IBStream setState returned {status}")));
    }
    Ok(())
}
